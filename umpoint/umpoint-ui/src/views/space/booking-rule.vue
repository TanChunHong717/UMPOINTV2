<template>
  <div class="mod-space__booking-rule">
    <el-form :inline="true" :model="state.dataForm" @keyup.enter="state.getDataList()" class="header-form">
      ...
      <div>
        <el-form-item>
          <el-button v-if="state.hasPermission('space:default-booking-rule:update')" type="primary">Update Default Booking Rule</el-button>
        </el-form-item>
        <el-form-item>
          <el-button
    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" border @selection-change="state.dataListSelectionChangeHandle" @sort-change="state.dataListSortChangeHandle" style="width: 100%">
      ...
      <el-table-column label="Booking Rule" header-align="center" align="center">
        ...
        <el-table-column label="Price" header-align="center" align="center">
          ...
          <el-table-column prop="fourHoursPrice" label="4 Hours" header-align="center" align="center" sortable="custom"></el-table-column>
        ...
        <el-table-column label="Min Booking Hours" header-align="center" align="center">
          <template v-slot="scope">
            <div v-if="scope.row.spcBookingRuleDTO">{{scope.row.spcBookingRuleDTO.minBookingHours}}</div>
          </template>
        </el-table-column>
      </el-table-column>
      <el-table-column label="Actions" fixed="right" header-align="center" align="center" width="150">
        <template v-slot="scope">
          <el-button v-if="state.hasPermission('space:space:info')" type="primary" link @click="$router.push({name:`space-info`, params: {id:scope.row.row.id}})">Details</el-button>
          <el-button v-if="state.hasPermission('space:booking-rule:update')" type="primary" link @click="bookingRuleUpdateHandle(scope.row.row)">Update</el-button>
        </template>
      </el-table-column>
    </el-table>
