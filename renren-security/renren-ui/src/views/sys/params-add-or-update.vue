<template>
  <el-dialog v-model="visible" :title="!dataForm.id ? 'Add' : 'Update'" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="rules" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="120px">
      <el-form-item prop="paramCode" label="Code">
        <el-input v-model="dataForm.paramCode" placeholder="Code"></el-input>
      </el-form-item>
...
const rules = ref({
  paramCode: [{ required: true, message: "Require field can not be empty", trigger: "blur" }],
  paramValue: [{ required: true, message: "Require field can not be empty", trigger: "blur" }]
});
